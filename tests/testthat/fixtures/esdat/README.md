...
| " | 11 data rows: 7 for work order `XX1234567` (3 samples x mixed analytes incl. a `<`-prefixed, a `>`-prefixed, a text "Observation" result, and one µS/cm unit), 2 QC rows (`QC-000001` LCS-linked Fluoride, `QC-000002` MB-linked pH), 1 plain `YY0000001` foreign-work-order row (flagged `foreign_work_order`), 1 compound-SampleCode NCP cross-reference `ZZ9999999001_XX1234567` (parses `sample_type=NCP` per R-4.6; counted in `n_ncp_foreign` and dropped at assembly) | FIXTURES.md "Work orders & files -> ESdat" pinned table, reproduced verbatim (values, prefixes, units) |
