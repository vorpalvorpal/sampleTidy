...
#     only. The 7 analyses already on that method are NOT touched - their
#     values do not sit at this factor and could not be explained, so they are
#     left for a separate look rather than "corrected" on a guess.
#
...
.mig007_new_analytes <- list(
  list(name = "Decachlorobiphenyl", units = "%", type = "QC", CAS = "2051-24-3"),
  list(name = "Volume Purged", units = "L", type = "quality", CAS = NA_character_)
)
...
.mig007_check_preconditions <- function(con) {
  ...
  # (4) the analytes to create must NOT already exist.
...
.mig007_verify_rulings <- function(con) {
  ...
  for (spec in .mig007_new_analytes) {
    got <- DBI::dbGetQuery(con, "SELECT units, type FROM analyte WHERE name = ?",
    ...
    db_transaction(con, function(con) {
      ...
      for (spec in .mig007_new_analytes) {
        db_append(con, "analyte", data.frame(
          uuid = .uuid_fn(), name = spec$name, units = spec$units,
