...
# ---- R-9.13 the ALS-source gate: WITHDRAWN (A79) --------------------------
#
# `.ig_als_gate()` used to sit here. A74 (2026-08-01) held that an ACIRL water
# sheet could only be imported when we hold the ALS report it cites, because
# most of the sheet is ALS data copied by hand. A79 (Robin, 2026-08-02)
# withdrew that: the ACIRL report is NATA-certified, so its data imports on its
# own and a missing ALS source no longer withholds anything (it held back 17 of
# 154 real workbooks, 644 rows). Supersession now happens at reconcile - an
# ACIRL transcription is deleted when the ALS equivalent commits - rather than
# by refusing the file at the door.
#
# The citation plumbing the gate consumed is NOT dead: `report$als_work_orders`
# and `report$n_water_sheets` (PLAN-06 R-6.5b) are what A80 files an ACIRL
# workbook by, making the cited ALS work order a CHILD project of the ACIRL
# report number. The gate's tests were retargeted, not deleted, and now assert
# the opposite outcome - see test-ingest.R.
...
.ig_build_report <- function(pipeline, dry_run, snapshot_path, removed) {
  ...
  list(
    ...
    # `als_gated` used to sit here. A79 withdrew the gate, so nothing is ever
    # withheld for a missing ALS source and the field would always be empty -
    # a permanently-zero count in the report reads as a live check that keeps
    # passing, which is worse than no field at all.
