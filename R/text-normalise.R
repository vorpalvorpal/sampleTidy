...
#     printed byte-by-byte (e.g. a tool renders the raw bytes of "°" as
#     the four characters `<c2><b0>` instead of decoding them).
#
...
#     MacRoman/latin-1 degree byte (0xA1, which decodes as U+00A1 "¡"
#     when misread as latin-1) and the classic cp1252 pairs from the old
#     `unify_value()` unit_dictionary, where a lost multibyte sequence
#     collapses to a single U+FFFD replacement character.
#
...
.lab_text_real_char_fixes <- c(
  "¡"       = "°", # MacRoman degree byte (0xA1 read as latin-1 -> ¡) -> °
  "�S/cm"   = "µS/cm", # cp1252 micro-sign loss -> µS/cm
  "�g/L"    = "µg/L",  # cp1252 micro-sign loss -> µg/L
  "�C"      = "°C"     # cp1252 degree-sign loss -> °C
)
...
#' ASCII-escape strings like `<c2><b0>C` (where `°C` was intended), or
