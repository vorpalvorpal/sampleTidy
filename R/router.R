...
# `.st_is_retryable_verdict()` used to sit here: the flagless re-decision that
# let an A74-gated file re-process once its ALS report arrived. A79 withdrew
# the gate, so nothing writes `als_source_missing` any more and this was the
# reason's only reader. Removed rather than kept: no `ingest_file` row in the
# live database has ever carried that reason (the gate never ran against the
# real corpus), so there is nothing left to un-quarantine, and an unreachable
# retry branch is one more way for a future `quarantined` reason to be
# re-decided by accident. `reconsider = TRUE` remains the only way back out of
# a router verdict.
